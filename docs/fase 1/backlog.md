...
| 7 | O `db:migrate` grava a exportação do `prod` com permissão `-rw-r--r--`. Ela tem os dados reais da casa. | `M1.2-ambientes-e-migracoes`, coleta da evidência do item 10 | O contrato não fixava permissão. O condutor pediu ao operador que fechasse o arquivo à mão com `chmod 600` |
| 8 | O `README.md` passa ao `psql` o caminho relativo `apps/api/certs/supabase-ca.crt`, sem dizer que o comando roda da raiz do repositório. O nome da pasta, `Casa Automática`, tem espaço e quebra o caminho absoluto sem aspas. | `M1.2-ambientes-e-migracoes`, coleta da evidência do item 10 | Achado depois da revisão. Mudança de `README.md` passa pelo ciclo de uma unidade |
