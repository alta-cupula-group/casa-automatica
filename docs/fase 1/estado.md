...
| 10 | Ambientes Supabase criados | **resolvida.** Em 2026-09-16 o operador criou as duas organizações e os dois projetos, com a Data API desligada. O `prod` foi recriado em São Paulo depois que o condutor apontou a latência da região do Canadá. Nomes e regiões em `docs/scope-brief.md`, seção 4. Os outros dois moradores entraram nas duas organizações. Nenhuma tarefa de painel fica pendente |
...
| 10 | Projetos Supabase apagados e a recriar | **aberta.** O operador apagou `omgheudterjqrjunpack` e os demais projetos em 2026-09-16. `dev` e `prod` precisam ser criados de novo, em organizações gratuitas separadas. Trava a `M1.2`. A decisão de 2026-09-15 segue valendo; muda só o ponto de partida. Registrado em `docs/scope-brief.md` e no fim de `docs/fase 1/unidades/M1.1-validar-supabase/exploracao.md` |
