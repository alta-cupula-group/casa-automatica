...
Infraestrutura disponível: servidor próprio em casa e domínio próprio. Desde setembro de 2026 o servidor roda Proxmox VE, com dois nodes: o `pve`, um notebook Dell Inspiron 15-3567 com Core i5-7200U, 24 GB de RAM e um disco de 94 GB, e o `pve2`, um desktop com GPU, ainda fora do cluster. A mesma máquina hospeda outros serviços da casa, como o servidor de Minecraft do grupo.
...
- **Onde o Docker roda:** numa **máquina virtual** do Proxmox, nunca no host e nunca num container LXC. Decidido pelo operador em 16/09/2026. Motivo: a VM tem kernel próprio, então um comprometimento dela não alcança o hipervisor nem os outros serviços da casa, e o container LXC exigiria privilégio para rodar Docker. O node `pve` tem memória sobrando, e esta parte do projeto não usa GPU. A VM se chama `casa-automatica`, roda Debian 13, e tem 2 vCPU, 4 GiB de RAM e 32 GB de disco.
- **Rede da casa:** o IP público é dinâmico e troca várias vezes por dia. A conexão é PPPoE, o que limita o MTU a 1492 em toda interface, inclusive dentro de cada guest. O Cloudflare Tunnel existe para o domínio não depender do IP.
...
- **Recursos limitados:** o node `pve` tem dois núcleos e quatro threads, e divide CPU, disco e rede de 100 Mb/s com os outros serviços da casa. Nada no MVP pode exigir GPU, muita memória ou paralelismo pesado.
