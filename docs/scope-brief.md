...
- **Casa como entidade e como tenant** (`House`), com chave estrangeira nas tabelas centrais. O MVP tem a casa real e uma casa de teste no mesmo banco, decidido em 13/09/2026. Nenhum dado atravessa de uma casa para outra.
...
- **Banco:** Supabase (Postgres). **Um projeto só**, decidido pelo operador em 13/09/2026, substituindo a decisão de dois projetos de 12/09/2026. A separação entre dado real e dado de teste é por casa, não por projeto: existe uma casa de teste, usada para movimentações irreais, ao lado da casa real. Motivo: dois projetos na mesma organização não protegem o dado real, e a casa já é a fronteira dos dados. Consequências: migração aplicada no projeto atinge o dado real direto, então ela roda antes num Postgres local e o banco é exportado antes de cada migração; testes automatizados continuam contra Postgres local, nunca contra o projeto; toda consulta filtra por casa. Supabase local via CLI/Docker é opcional para quem quiser. Migrações versionadas no repositório. O plano gratuito permite dois projetos ativos por pessoa, validado em 12/09/2026 na unidade `M1.1`. Projetos gratuitos pausam após 7 dias sem uso; reativar é um clique.
